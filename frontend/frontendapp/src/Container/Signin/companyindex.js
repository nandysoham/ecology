...
export default function Signin(props) {
    ...
    const submitsignin = async(email,password)=>{
        ...
        const response = await fetch("http://localhost:2000/api/company/login",{
        ...
        if(token.success){
            ...
            localStorage.setItem("companytoken",token.authtoken);
            ...
            <Layout mode = {props.mode} Togglemode = {props.Togglemode}>
                <div style={{
                    ...
                    <div className="container">
                        <h2 className="text-center " style={{color:"white", paddingTop:"80px"}}> NGO Login</h2>
                    </div>
